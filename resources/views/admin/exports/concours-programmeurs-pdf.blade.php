...
<div class="meta">
    ...
    <strong>Nombre total de participants :</strong> {{ count($participants) }}
</div>

<table class="stats-bar">
    <tr>
        <td><div class="stat-num">{{ count($participants) }}</div><div class="stat-lbl">Participants</div></td>
        @if($lycee > 0)
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                ...
                @elseif($c === 'type_concours')
                    <span class="badge {{ $p->type_concours === 'CMPL' ? 'badge-lycee' : 'badge-sup' }}">
                        {{ $p->type_concours === 'CMPL' ? 'Lycée' : 'Supérieur' }}
                    </span>
                @else
                    {{ $p->$c ?? '—' }}
                @endif
