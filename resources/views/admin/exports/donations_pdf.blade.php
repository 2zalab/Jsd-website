...
<head>
    <meta charset="utf-8">
    <title>Journées Sahel Digital - Liste des donateurs</title>
...
<strong>Nombre total de donateurs :</strong> {{ $donations->count() }} <br>
<strong>Montant total :</strong> {{ number_format($donations->sum('amount'), 0, ',', ' ') }} FCFA
</p>
...
<tr>
    ...
    <td>{{ $don->name ?? '—' }}</td>
    <td>{{ $don->email ?? '—' }}</td>
    <td>{{ number_format($don->amount, 0, ',', ' ') }}</td>
</tr>
@endforeach
