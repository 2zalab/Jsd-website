<div class="p-6">
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Concours Meilleurs Programmeurs — Lycée (CMPL)</h1>
        <a href="{{ route('admin.cmpl.pdf') }}" target="_blank"
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="overflow-x-auto">
            <table id="cmplTable" class="min-w-full">
                <thead class="bg-indigo-600 text-white">
                    <tr>
                        ...
                        <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider cursor-pointer">Classe</th>
        ...
        <div class="px-5 py-4 bg-gray-50 border-t flex items-center justify-between text-sm text-gray-600">
            <span>Affichage de <span id="startIndex">1</span> à <span id="endIndex">{{ count($cmpl) }}</span> sur <span id="totalEntries">{{ count($cmpl) }}</span> entrées</span>
...
(function() {
    ...
    const colMap = {nom:0, etablissement:1, classe:2, langages:3};
